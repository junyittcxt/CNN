...
def get_and_clean_data_mysql(query_date, raw_data_file, db_variant, SEQ_LEN, BREAKOUT_WINDOW, TARGET_TO_PREDICT, CLEAN_METHOD_X, accepted_index):
    ...
    while nrows < SEQ_LEN:
        ...
        if query_df.shape[0] == 0:
            output = query_df.index[-1], 0, -1, "Not enough data: 0 row."
            prediction_dict = create_prediction_dict(output, query_date)
            return -1, prediction_dict
            break
...
def get_signal(query_date, db_variant, PATH_DICT, key, use_mysql = True, mongo_db = "Production", price_coll = "prices"):
    try:
        output = None
        ...
        # Get and Clean Data
        # Prices from MySQL
        status, df = get_and_clean_data_mysql(query_date, raw_data_file, db_variant, SEQ_LEN, BREAKOUT_WINDOW, TARGET_TO_PREDICT, CLEAN_METHOD_X, accepted_index)
        if status == -1:
            return df
